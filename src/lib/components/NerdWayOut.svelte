...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h2 class="text-2xl font-bold font-serif text-ink-900 tracking-wide">Nerd Way Out</h2>
	...
	<div class="space-y-4">
		<h3 class="text-lg font-semibold font-serif text-ink-900">Score Specification Values</h3>
		...
		<div class="space-y-2">
			{#each criteria as criterion}
				...
				<div class="border-2 border-ink-200 rounded-lg bg-parchment-50">
					<button
						on:click={() => toggleExpand(criterion.id)}
						class="w-full flex items-center justify-between p-4 text-left hover:bg-parchment-100 focus:outline-none focus:bg-parchment-100 rounded-lg transition-colors"
					>
						<div class="flex-1">
							<div class="font-semibold text-lg text-ink-900">{criterion.name}</div>
							{#if criterion.description}
								<div class="text-sm text-charcoal-light mt-1">{criterion.description}</div>
							{/if}
							<div class="text-xs text-charcoal-lighter mt-1">
					...
					{#if isExpanded}
						<div class="border-t-2 border-ink-200 p-4 bg-parchment-100">
							<div class="space-y-3">
								<h4 class="font-medium text-ink-900">Score Each Value (1-5 scale)</h4>
								<p class="text-sm text-charcoal-light">1 = Poor, 3 = Average, 5 = Excellent</p>
								<div class="grid gap-3">
									{#each Object.entries(criterion.values) as [valueKey, valueData]}
										<div class="flex items-center justify-between p-3 bg-parchment-50 border-2 border-ink-100 rounded-lg">
											<div class="flex-1">
												<div class="font-medium text-charcoal">{valueData.value}</div>
											</div>
											<div class="flex items-center space-x-2">
												<input
													...
													class="w-24 accent-brass-600"
												/>
												<span class="w-8 text-sm text-center font-mono text-ink-900">{valueData.score}</span>
											</div>
										</div>
									{/each}
								</div>
